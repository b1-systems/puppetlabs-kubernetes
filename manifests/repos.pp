    ...
    case $facts['os']['family'] {
      'Debian': {
        ...
        if $container_runtime == 'docker' and $manage_docker == true {
          apt::source { 'docker':
            location => pick($docker_apt_location,'https://download.docker.com/linux/debian'),
            repos    => pick($docker_apt_repos,'main'),
            release  => pick($docker_apt_release, $facts['os']['distro']['codename']),
            key      => {
              'id'     => pick($docker_key_id,'9DC858229FC7DD38854AE2D88D81803C0EBFCD88'),
              'source' => pick($docker_key_source,'https://download.docker.com/linux/debian/gpg'),
            },
          }
        }
      }
